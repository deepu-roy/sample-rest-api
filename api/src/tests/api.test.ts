...
import { db, initializeDatabase } from '../db/init';
...
describe('API Endpoints', () => {
    ...
    describe('Users API', () => {
        ...
        it('should list users with pagination and include role information', async () => {
            const res = await request(app).get('/api/users?page=1&per_page=2').expect(200);
        ...
        it('should create a new user with default role', async () => {
            ...
            const userRes = await request(app).get(`/api/users/${createdUserId}`).expect(200);
        ...
        it('should create a new user with specified role', async () => {
            ...
            const userRes = await request(app).get(`/api/users/${res.body.id}`).expect(200);
        ...
        it('should get a user by ID with role information', async () => {
            const res = await request(app).get(`/api/users/${createdUserId}`).expect(200);
        ...
        it('should reject update with no valid fields', async () => {
            const res = await request(app).put(`/api/users/${createdUserId}`).send({}).expect(400);
        ...
        it('should return 404 when updating non-existent user', async () => {
            ...
            const res = await request(app).put('/api/users/999999').send(updateData).expect(404);
        ...
        describe('Role Filtering', () => {
            let userRole1Id: number, userRole2Id: number, userRole3Id: number;
            ...
            it('should filter users by role ID', async () => {
                ...
                res.body.data.forEach((user: { role_id: number; role: { name: string } }) => {
                ...
                const testUser = res.body.data.find(
                    (user: { id: number }) => user.id === userRole1Id
                );
            ...
            it('should filter users by admin role', async () => {
                ...
                // Should include our test user
                const testUser = res.body.data.find(
                    (user: { id: number }) => user.id === userRole2Id
                );
                expect(testUser).toBeDefined();
            });
            ...
            it('should reject invalid role parameter - non-numeric', async () => {
                const res = await request(app).get('/api/users?role=invalid').expect(400);
    ...
    describe('Roles API', () => {
        ...
        it('should get a specific role by ID', async () => {
            ...
            const res = await request(app).get(`/api/roles/${firstRole.id}`).expect(200);
        ...
        it('should return 400 for invalid role ID', async () => {
            ...
            for (const invalidId of invalidIds) {
                const res = await request(app).get(`/api/roles/${invalidId}`).expect(400);
        ...
        it('should verify default roles exist', async () => {
            ...
            const roleNames = res.body.data.map((role: { name: string }) => role.name);
